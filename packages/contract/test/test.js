...
describe('Web3Mint', () => {
    ...
    it('Should revert when minting over max supply', async () => {
        const Mint = await ethers.getContractFactory('Web3Mint');
        const mintContract = await Mint.deploy();
        await mintContract.deployed();
        ...
        await expect(mintContract.connect(owner).mintIpfsNFT(nftName, ipfsCID)).to.be.revertedWith("Exceeds max supply.");
    });

    it('totalSupply() should return correct value', async () => {
        const Mint = await ethers.getContractFactory('Web3Mint');
        const mintContract = await Mint.deploy();
        await mintContract.deployed();
        ...
        assert.equal(
            await mintContract.totalSupply(),
            5,
        );
